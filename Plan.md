...
Scheduler (cron/worker) enqueues a discover_jobs task.



JobDiscoveryAgent fetches listings from sources and emits raw artifacts.



For each artifact, enqueue parse_job.



JobParsingAgent normalizes and saves to jobs, then enqueues analyze_job.



JobAnalysisAgent enriches the job (summary, tags, embeddings) and updates jobs.
...
User selects a job via UI or API.



API creates an applications record (status draft).



Orchestrator triggers ResumeTailoringAgent for this job.



Tailored bullets & cover letter are saved in generated_contents.



UI shows drafts; user can edit/resave.
...
ApplicationAssistantAgent determines necessary fields/questions (from configured templates + heuristics).



ResponseMemoryAgent and LLM generate draft answers and attach to generated_contents.



System creates a pending_actions record of type submit_application with a full plan (target URL, job id, portal type, answers, resume path).



UI displays this action; user reviews content and clicks Approve.



On approval, a worker consumes the action and runs Playwright to execute it.



Upon success/failure, the worker updates applications.status and logs an event.
...
Given a job and company domain, CompanyCareersAgent looks for /careers, /jobs, or configured endpoints.



Scrapes relevant openings and matches them semantically to the original role.



If a match is found, creates additional jobs/applications and follows the same tailoring & pending-action path.
...
User requests outreach suggestions for a job.



OutreachAgent retrieves job + profile KB + templates.



Generates N candidate messages (connection, follow-up, referral) saved in generated_contents.



Optionally creates pending_actions (e.g. send_linkedin_message) that the user can use as copy-paste helpers rather than auto-send.
...
Profile knowledge & response memory ingestion
...
Job discovery & parsing pipeline
...
Application assistant & pending actions
...
Company careers and outreach workflows
...
Application tracking & summaries
...
Hardening, logging, and monitoring
